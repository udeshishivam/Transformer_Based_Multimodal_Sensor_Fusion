...
def eval_model_iou(version,
                modelf,
                dataroot='/home/kaushek/TFGrid/Dataset',
                gpuid=1,
                config='',
                H=900, W=1600,
                resize_lim=(0.193, 0.225),
    ...
    # Configuration for grid and data augmentation
    grid_conf = {
        'xbound': xbound,
        'ybound': ybound,
        'zbound': zbound,
    ...
    # Compile the model
    model = compile_model(cfg.grid_conf, cfg.data_aug_conf, outC=cfg.num_classes, cfg_pp=cfg.cfg_pp, tf_config=cfg)
    model.to(device)
    
    # Load the checkpoint
    print('loading', modelf)
    checkpoint = torch.load(modelf)  # Load the checkpoint
    model.load_state_dict(checkpoint['model_state_dict'])  # Assuming model weights are stored under 'model_state_dict'
    
    # Set loss function (if needed)
    loss_fn = SimpleLoss(1.0).cuda(gpuid)  # Ensure it's on the same device
    
    # Set model to evaluation mode
    model.eval()
    cfg.cfg_pp['grid_conf'] = grid_conf
    # Perform evaluation
    val_info = get_val_info(model, valloader, loss_fn, device, cfg.cfg_pp, print_img=True, weather_class=0)
    print(val_info)
